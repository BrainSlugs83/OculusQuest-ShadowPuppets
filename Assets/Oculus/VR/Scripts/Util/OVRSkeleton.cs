...
public class OVRSkeleton : MonoBehaviour
{
    ...
    private void Initialize()
    {
        ...
        if (OVRPlugin.GetSkeleton((OVRPlugin.SkeletonType)_skeletonType, out skeleton))
        {
            ...
            for (int i = 0; i < skeleton.NumBones; ++i)
            {
                ...
                var pos = skeleton.Bones[i].Pose.Position.FromFlippedZVector3f();
                var rot = skeleton.Bones[i].Pose.Orientation.FromFlippedZQuatf();
            ...
            if (_enablePhysicsCapsules)
            {
                ...
                for (int i = 0; i < skeleton.NumBoneCapsules; ++i)
                {
                    ...
                    var bone = Bones[capsule.BoneIndex].Transform;
    ...
    private void Update()
    {
        ...
        if (data.IsDataValid)
        {
            lerpWeight = data.IsDataHighConfidence ? 0.99f : Mathf.Clamp(lerpWeight * .985f, 0.04f, 0.1f);
            ...
            {
                _bones[i].Transform.localRotation = Quaternion.Lerp
                (
                    _bones[i].Transform.localRotation,
                    data.BoneRotations[i].FromFlippedZQuatf(),
                    lerpWeight
                );
            }
        }
    ...
    private void FixedUpdate()
    {
        ...
        if (_enablePhysicsCapsules)
        {
            ...
            for (int i = 0; i < _capsules.Count; ++i)
            {
                ...
                {
                    var bone = _bones[(int)capsule.BoneIndex].Transform;
