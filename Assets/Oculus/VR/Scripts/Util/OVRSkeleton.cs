...
public class OVRSkeleton : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (data.IsDataValid)
		{
            float lerpWeight = data.IsDataHighConfidence ? 1.0f : 0.1f;

			if (_updateRootPose)
			{
				transform.localPosition = Vector3.Lerp(transform.localPosition, data.RootPose.Position.FromFlippedZVector3f(), lerpWeight);
				transform.localRotation = Quaternion.Lerp(transform.localRotation, data.RootPose.Orientation.FromFlippedZQuatf(), lerpWeight);
			}

			if (_updateRootScale)
			{
                transform.localScale = Vector3.Lerp(transform.localScale, new Vector3(data.RootScale, data.RootScale, data.RootScale), lerpWeight);

            }

			for (var i = 0; i < _bones.Count; ++i)
			{
				_bones[i].Transform.localRotation = Quaternion.Lerp(_bones[i].Transform.localRotation, data.BoneRotations[i].FromFlippedZQuatf(), lerpWeight);
			}
		}
	}
